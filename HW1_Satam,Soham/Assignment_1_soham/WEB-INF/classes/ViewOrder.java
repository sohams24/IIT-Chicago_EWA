...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		{
			// if(utility.usertype().equalsIgnoreCase("manager"))
			// {
			// 	pw.print("<form name ='ViewOrder' action='ViewOrder' method='get'>");
			// 	pw.print("<div id='content'><div class='post'><h2 class='title meta'>");
			// 	pw.print("<a style='font-size: 24px;'>Order</a>");
			...
			// 		pw.print("<td>Delivery/Pickup:</td></tr>");

			// 		for(ArrayList<OrderPayment> ops : orderPayments.values())
			// 		{
			// 			for(OrderPayment op : ops)
			// 			{
			// 				pw.print("<tr>");			
			// 				pw.print("<td><input type='radio' name='orderName' value='"+op.getOrderName()+"'></td>");			
			// 				pw.print("<td>"+op.getOrderId()+".</td><td>"+op.getUserName()+"</td><td>"+op.getOrderName()+"</td><td>Price: "+op.getOrderPrice()+"</td><td>"+op.getOrderDate()+"</td><td>"+op.getOrderDate().plusDays(14)+"</td>");
			// 				if(op.getPickup()){
			...
			// 	else
			// 	{
			// 		pw.print("<h4 style='color:red'>There are no orders placed.</h4>");
			// 	}
			// }
			// else
			// {
			...
			{	
				...
				{	
					...
					for (OrderPayment oi : orderPayments.get(orderId)) 
					{
						...
						pw.print("<td>"+oi.getOrderId()+".</td><td>"+oi.getUserName()+"</td><td>"+oi.getOrderName()+"</td><td>Price: "+oi.getOrderPrice()+"</td><td>"+oi.getOrderDate()+"</td><td>"+oi.getOrderDate().plusDays(14)+"</td>");
						if(oi.getPickup()){
							pw.print("<td>Pickup</td>");
						}
						else{
