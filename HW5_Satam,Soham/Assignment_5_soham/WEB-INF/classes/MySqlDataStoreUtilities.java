...
{   String msg = "Product is deleted successfully";
    ...
    public static void insertTransaction(Transaction t){
        try {
            System.out.println("Trying to insert order into transactions table.");
            getConnection();
            String insertIntoCustomerOrderQuery = "INSERT INTO transactions(loginId,customerName,customerAge,customerOcc,creditCardNo,orderId,orderDate," +
                "exptDelDate,actDelDate,productId,productName,category,manufacturer,reviewRating,delTrackId,delType,delZipCode,transactionStatus,orderReturned,orderDelOnTime)" +
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";

            PreparedStatement pst = conn.prepareStatement(insertIntoCustomerOrderQuery);
            pst.setString(1, t.getLoginId());
            pst.setString(2, t.getCustName());
            pst.setInt(3, t.getCustAge());
            pst.setString(4, t.getCustOcc());
            pst.setString(5, t.getCreditCardNo());
            pst.setInt(6, t.getOrderId());
            Date orderDate = Date.valueOf(t.getOrderDate());
            pst.setDate(7, orderDate);          
            Date exptDelDate = Date.valueOf(t.getExptDelDate());
            pst.setDate(8, exptDelDate);
            Date actDelDate = Date.valueOf(t.getActDelDate());
            pst.setDate(9, actDelDate);
            pst.setString(10, t.getProductId());
            pst.setString(11, t.getProductName()); 
            pst.setString(12, t.getCategory());
            pst.setString(13, t.getManufacturer());
            pst.setInt(14, t.getReviewRating());
            pst.setString(15, t.getDelTrackId());
            pst.setString(16, t.getDelType());
            pst.setString(17, t.getDelZipCode());
            pst.setBoolean(18, t.getTranStatus());
            pst.setBoolean(19, t.getOrderReturned());
            pst.setBoolean(20, t.getOrderDelOnTime());
            pst.execute();
        } catch (Exception e) {
            System.out.println(e);
            System.out.println("Transaction insert unsuccessful");
        }
    }
